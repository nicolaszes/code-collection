...
handlers._users.post = (data, callback) => {
  console.log(28, data.payload)
  // check that all required fields and filled out
...
handlers._users.put = (data, callback) => {
  ...
  var phone = typeof(data.payload.phone) == 'string' && data.payload.phone.trim().length == 11 
    ? data.payload.phone.trim() 
    : false
...
handlers._tokens.post = function(data,callback){
  ...
  if (phone && password) {
    // look up the users who matches that phone number
    _data.read('users', phone, (err, userData) => {
      if (!err && userData) {
        const hashedPassword = helpers.hash(password)
