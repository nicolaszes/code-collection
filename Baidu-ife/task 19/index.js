window.onload = function(argument) {
    ...
    function renderRange(left, right) {
        ...
        queue.push(function() {
            if (curLeft >= 0) {
                $$('#numContainer li')[curLeft].style.border = 'none';
            }
            if (curRight >= 0) {
                $$('#numContainer li')[curRight].style.border = 'none';
            }

            curLeft = left;
            curRight = right;
            $$('#numContainer li')[curLeft].style.border = '3px solid #f00';
            $$('#numContainer li')[curRight].style.border = '3px solid #f00';
        });
    }

    function renderSearch(index, isSearchEnd) {
        queue.push(function() {
            $$('#numContainer li')[index].style.backgroundColor = 'greenyellow';
        });
        if (!isSearchEnd) {
            var num = arrData[index];
            queue.push(function() {
                $$('#numContainer li')[index].style.backgroundColor = getBGColor(num);
            });
        }
    }
    ...
    function renderSwap(idx1, idx2) {
        ...
        queue.push(function() {
            $$('#numContainer li')[idx1].style.backgroundColor = getBGColor(num1);
            $$('#numContainer li')[idx2].style.backgroundColor = getBGColor(num2);

            $$('#numContainer li')[idx1].style.height = (num1 * 2) + 'px';
            $$('#numContainer li')[idx2].style.height = (num2 * 2) + 'px';

            $$('#numContainer li')[idx1].innerHTML = num1;
            $$('#numContainer li')[idx2].innerHTML = num2;
        });
    }
    ...
    function render() {
        $('#numContainer').innerHTML = arrData.map(function(d) {
            return "<li style = 'height:" + (d * 2) + 'px; background-color:' + getBGColor(d) + "';>" + d + '</li>';
        }).join('');
    }
